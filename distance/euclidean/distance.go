...
type euclideanDistanceImpl[TV interfaces.VectorType] struct {
	nodeSize       int
	maxNumChildren interfaces.ItemID
	vectorLength   int
}

func Distance[TV interfaces.VectorType](vectorLength int) *euclideanDistanceImpl[TV] {
	n := minkowskiNodeImpl[TV]{}
	d := &euclideanDistanceImpl[TV]{
...
func (d *euclideanDistanceImpl[TV]) PreProcess(nodes unsafe.Pointer, nodeCount interfaces.ItemID) {}

func (d *euclideanDistanceImpl[TV]) Distance(x interfaces.Node[TV], y interfaces.Node[TV]) TV {
	return vector.EuclideanDistance(x.GetVector(d.vectorLength), y.GetVector(d.vectorLength), d.vectorLength)
}

func (d *euclideanDistanceImpl[TV]) Normalize(node interfaces.Node[TV]) {
...
func (d *euclideanDistanceImpl[TV]) Margin(n interfaces.Node[TV], y []TV) TV {
...
func (d *euclideanDistanceImpl[TV]) CreateSplit(nodes []interfaces.Node[TV], nodeSize int, random interfaces.Random, n interfaces.Node[TV]) {
...
func (d *euclideanDistanceImpl[TV]) Side(n interfaces.Node[TV], y []TV, random interfaces.Random) interfaces.Side {
	margin := d.Margin(n, y)
	if margin != 0 {
		if margin > 0 {
...
func (d *euclideanDistanceImpl[TV]) MapNodeToMemory(mem unsafe.Pointer, itemIndex interfaces.ItemID) interfaces.Node[TV] {
	pos := unsafe.Add(mem, uintptr(itemIndex)*uintptr(d.nodeSize))
	return (*minkowskiNodeImpl[TV])(pos)
}

func (d *euclideanDistanceImpl[TV]) PQDistance(distance, margin TV, side interfaces.Side) TV {
...
func (d *euclideanDistanceImpl[TV]) NormalizedDistance(distance TV) TV {
	return TV(math.Sqrt(math.Max(float64(distance), 0)))
}

func (d *euclideanDistanceImpl[TV]) PQInitialValue() TV {
	return TV(math.Inf(1))
}

func (d *euclideanDistanceImpl[TV]) InitNode(node interfaces.Node[TV]) {}

func (d *euclideanDistanceImpl[TV]) MaxNumChildren() interfaces.ItemID {
	return d.maxNumChildren
}

func (d *euclideanDistanceImpl[TV]) NodeSize() int {
	return d.nodeSize
}

func (d *euclideanDistanceImpl[TV]) VectorLength() int {
